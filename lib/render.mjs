...
export class TurnAccumulator {
  ...
  footerLine() {
    ...
    return `=== status=${this.status} tokens=${formatTokenUsage(this.tokenUsage)} durationMs=${durationMs} ===`;
  }
  ...
  buildReport(reportLevel) {
    const body = this.finalMessage || this.errorMessage || `(turn ${this.status})`;
    const footer = [
      `status: ${this.status}`,
      `files: ${this.filesTouched.size ? [...this.filesTouched].sort().join(", ") : "none"}`,

...
function errorInfoName(info) {
  ...
  const [name, detail] = Object.entries(info)[0] ?? [];
  if (!name) {
    return null;
  }
...
const GOAL_STOPPED_SHORT = new Set(["paused", "blocked", "usageLimited", "budgetLimited"]);

export function formatGoal(goal, label = null) {
  if (!goal) {
    return "no goal set";
  }
  const lines = [`goal: ${goal.status} — ${oneLine(goal.objective, 300)}`];
  if (GOAL_STOPPED_SHORT.has(goal.status)) {
    const resume = label ? `ception goal ${label} --resume` : "ception goal <label> --resume";
    lines.push(`the goal stopped before completing; restart the run with \`${resume}\``);
  }
  return lines.join("\n");
}
...
export class TurnAccumulator {
  ...
  footerLine() {
    ...
    const errorInfo = this.errorInfo ? ` errorCode=${this.errorInfo}` : "";
    return `=== status=${this.status}${errorInfo}${compactions} tokens=${formatTokenUsage(this.tokenUsage)} durationMs=${durationMs} ===`;
  }
  ...
  handleNotification(method, params) {
    switch (method) {
      ...
      case "turn/completed":
        ...
        this.errorInfo = errorInfoName(params.turn?.error?.codexErrorInfo);
    ...
    switch (item.type) {
      ...
      case "reasoning": {
        ...
        const deltaContent = [...this.reasoningTextDeltas.entries()]
          ...
          .map(([, value]) => value)
          .join("\n");
        // The completed item repeats what the deltas streamed; use it as the
        // source of truth and fall back to deltas, like agentMessage above.
        const summary = asTextList(item.summary).join("\n") || deltaSummary;
        const content = asTextList(item.content).join("\n") || deltaContent;
  ...
  buildReport(reportLevel) {
    let body = this.finalMessage || this.errorMessage || "";
    ...
    const footer = [
      `status: ${this.status}`,
      ...(this.errorInfo ? [`error code: ${this.errorInfo}`] : []),
    ...
    const noMessage = `(turn ${this.status}, no final message)`;
    if (reportLevel === "items") {
      return [body || noMessage, "", ...this.itemLines, "", footer].join("\n");
    }
    // Turns codex starts for a goal often end without an agent message. An
    // almost empty report is worse than a terse trail of what it did — but
    // brief must stay brief, so the trail is what it touched, not how it
    // thought.
    if (!body) {
      const trail = this.itemLines
        .filter((line) => !line.startsWith("[reasoning]"))
        .slice(-20)
        .map((line) => oneLine(line, 200));
      body = [noMessage, ...trail].join("\n");
    }
    return `${body}\n\n${footer}`;
  }
}

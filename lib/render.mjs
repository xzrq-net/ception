...
export class TurnAccumulator {
  ...
  handleNotification(method, params) {
    switch (method) {
      ...
      case "turn/completed":
        ...
        // Amnesia signature: the model ended its turn straight out of a
        // mid-turn context compaction without doing any work afterwards.
        // Codex acknowledges its instructions ("Instructions loaded") and
        // stops, so a lost turn would otherwise report as completed.
        if (this.status === "completed" && this.compactions > 0 && !this.workSinceCompaction) {
          this.status = "failed";
  ...
  markWork(itemType) {
    if (itemType !== "agentMessage" && itemType !== "reasoning" && itemType !== "contextCompaction") {
      this.workSinceCompaction = true;
    }
  }
  ...
  handleItemCompleted(item) {
    ...
    switch (item.type) {
      case "contextCompaction": {
        this.compactions += 1;
        this.workSinceCompaction = false;

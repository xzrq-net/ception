...
class Daemon {
  ...
  async commandInterrupt(socket) {
    ...
    try {
      // Interrupting a turn under an active goal only frees the thread for the
      // goal's next turn, so interrupt would not stop the run. Pause the goal
      // first and say so — the operator asked for the work to stop.

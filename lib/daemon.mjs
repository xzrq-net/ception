...
import { sandboxModeToPolicy, updateLabelState } from "./state.mjs";
...
class Daemon {
  ...
  async start() {
    ...
    this.app.onServerRequest((warning, message) => this.handleServerRequest(warning, message));
  ...
  async startTurn(socket, command) {
    ...
    const params = {
      ...
      cwd: this.options.cwd,
      approvalPolicy: "never",
      sandboxPolicy: sandboxModeToPolicy(this.options.sandbox, this.options.cwd)
    };
    if (this.options.model) {
      params.model = this.options.model;
    }
  ...
  handleServerRequest(warning, message) {
    const line = `[warning] ${warning}`;
    this.log(`${line}: ${JSON.stringify(message)}`);
    if (this.activeTurn) {
      for (const client of this.activeTurn.clients) {
        writeJson(client.socket, { type: "warning", message: warning });
      }
    }
  }

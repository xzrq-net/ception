...
class Daemon {
  ...
  async ensureThread() {
    ...
    try {
      ...
      // A daemon the state file does not know about is invisible to list and
      // cannot be resumed; fail the client and go away instead of lingering.
      this.threadId = null;
      this.log(`[state] ${error.message}; shutting down`);
      setImmediate(() => this.shutdown("label not persisted").catch(() => {}));
      throw error;
    }

...
class Daemon {
  ...
  completeActiveTurn() {
    ...
    // compaction on older app-servers, where no goal was involved. Only
    // compactions since the last hold count — the cumulative total would make
    // every continuation pay a second grace window after finishing clean.
  ...
  adoptOrphanTurn(turnId) {
    ...
    const accumulator = new TurnAccumulator({
      ...
      threadId: this.threadId,
      turnId,
      prompt: "(unattended continuation)",
      logPath: this.options.logPath
    });

...
async function writeProjectStateByPath(statePath, state) {
  ...
  // Not keyed on pid: another container can be running the same pid.
  const tempPath = `${statePath}.${crypto.randomBytes(6).toString("hex")}.tmp`;
...
async function withStateLock(statePath, fn, timeoutMs = 10000) {
  ...
  await fs.mkdir(path.dirname(statePath), { recursive: true, mode: 0o700 });
  // The critical section is one read-modify-write of a small file, so a
  // holder from another container that is older than a few seconds is debris.
  const release = await acquirePidLock(lockPath, { timeoutMs, staleMs: 5000, onBusy: () => sleep(25) });
  try {
    return await fn();
  } finally {
    await release();
...
// Temp files of state writes and lock publications that a SIGKILL cut short.
// Anything a holder is still using is seconds old; an hour is safe.
async function sweepTempFiles(dir, maxAgeMs = 3600_000) {
...
export async function gcProjectState(context) {
  await sweepTempFiles(context.stateDir);

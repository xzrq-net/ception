import { spawn } from "node:child_process";
import fsSync from "node:fs";
import fs from "node:fs/promises";
import net from "node:net";
import path from "node:path";
import process from "node:process";
import { fileURLToPath } from "node:url";

import { acquirePidLock, holderLine, pidAlive } from "./lock.mjs";
import { formatGoal, statusExitCode } from "./render.mjs";
import {
  contextFor,
  gcProjectState,
  listAllLabels,
  listProjectLabels,
  readLabelState,
  sessionAlive,
  takeLabel
} from "./state.mjs";

const BIN_PATH = fileURLToPath(new URL("../bin/ception.mjs", import.meta.url));

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

function usageError(message) {
  const error = new Error(message);
  error.exitCode = 4;
  return error;
}

function socketConnect(socketPath) {
  return new Promise((resolve, reject) => {
    const socket = net.createConnection({ path: socketPath });
    socket.once("connect", () => resolve(socket));
    socket.once("error", reject);
  });
}

async function waitForSocket(socketPath, timeoutMs = 15000, shouldAbort = null) {
  const deadline = Date.now() + timeoutMs;
  let lastError = null;
  while (Date.now() < deadline) {
    const abort = shouldAbort?.();
    if (abort) {
      throw abort;
    }
    try {
      const socket = await socketConnect(socketPath);
      socket.end();
      return;
    } catch (error) {
      lastError = error;
      if (error.code === "ECONNREFUSED") {
        await fs.rm(socketPath, { force: true }).catch(() => {});
      }
      await sleep(100);
    }
  }
  throw lastError ?? new Error(`timeout waiting for ${socketPath}`);
}

async function commandDaemon(socketPath, command) {
  const socket = await socketConnect(socketPath);
  socket.setEncoding("utf8");
  const result = await new Promise((resolve, reject) => {
    let buffer = "";
    let settled = false;
    const finish = (fn, value) => {
      if (settled) {
        return;
      }
      settled = true;
      fn(value);
    };
    socket.on("data", (chunk) => {
      buffer += chunk;
      let newline = buffer.indexOf("\n");
      while (newline !== -1) {
        const line = buffer.slice(0, newline);
        buffer = buffer.slice(newline + 1);
        newline = buffer.indexOf("\n");
        if (!line.trim()) {
          continue;
        }
        let message;
        try {
          message = JSON.parse(line);
        } catch (error) {
          finish(reject, new Error(`invalid daemon JSON: ${error.message}`));
          socket.destroy();
          return;
        }
        if (message.type === "error") {
          const error = new Error(message.message);
          error.exitCode = 4;
          finish(reject, error);
          socket.destroy();
          return;
        }
        if (message.type === "result") {
          finish(resolve, message);
          socket.end();
          return;
        }
      }
    });
    socket.on("error", (error) => finish(reject, error));
    socket.on("close", () => finish(reject, new Error("daemon connection closed before result")));
    socket.write(`${JSON.stringify(command)}\n`);
  });
  return result;
}

async function daemonStatus(contextOrRow) {
  try {
    const response = await commandDaemon(contextOrRow.socketPath, { cmd: "status" });
    return response.daemon ?? null;
  } catch {
    return null;
  }
}

async function acquireLock(context, timeoutMs = 15000) {
  return acquirePidLock(context.lockPath, {
    timeoutMs,
    // Held through daemon startup, which can sit through an npx download; a
    // holder we cannot verify (runtime dir on a shared mount) gets that long.
    staleMs: 10 * 60_000,
    // A live holder is another client spawning this daemon; once its socket
    // answers, return null — the caller has a daemon and nothing to spawn.
    onBusy: async () => {
      try {
        await waitForSocket(context.socketPath, 500);
        return null;
      } catch {
        await sleep(100);
      }
    }
  });
}

async function spawnDaemon(context, { resumeThreadId = null, model = null, effort = null, logPath = null } = {}) {
  const release = await acquireLock(context);
  if (release === null) {
    return { spawned: false };
  }

  try {
    // The lock may have become available because another spawner finished and
    // released it. Recheck while holding the lock before starting a second
    // daemon for the same label.
    if (await daemonStatus(context)) {
      return { spawned: false };
    }

    const daemonLogPath = logPath ?? context.logPath;
    const logFd = fsSync.openSync(daemonLogPath, "a");
    const args = [
      "--label",
      context.label,
      "--cwd",
      context.cwd,
      "--hash",
      context.hash,
      "--session",
      context.sessionKey,
      "--socket",
      context.socketPath,
      "--state",
      context.statePath,
      "--log",
      daemonLogPath
    ];
    if (resumeThreadId) {
      args.push("--resume-thread-id", resumeThreadId);
    }
    if (model) {
      args.push("--model", model);
    }
    if (effort) {
      args.push("--effort", effort);
    }

    const env = { ...process.env };
    if (context.watched) {
      env.CEPTION_CLAUDE_PID = String(context.watched.pid);
      env.CEPTION_CLAUDE_STARTTIME = String(context.watched.starttime);
    }

    // Spawn through an intermediate that exits immediately, so the daemon
    // reparents to init before this client blocks on the turn. A direct child
    // would die with the client: kills of the client's process tree (Claude
    // Code stopping the background shell) walk ppids, and detached/setsid
    // does not escape that.
    const child = spawn(process.execPath, [BIN_PATH, "spawn-orphan", ...args], {
      cwd: context.cwd,
      env,
      stdio: ["ignore", "pipe", logFd]
    });
    fsSync.closeSync(logFd);
    let spawnerOut = "";
    child.stdout.on("data", (chunk) => {
      spawnerOut += chunk;
    });
    // "close" and not "exit": exit can fire while pid bytes still sit in the
    // stdout pipe.
    const spawnerExit = await new Promise((resolve) => child.once("close", resolve));
    if (spawnerExit !== 0) {
      throw new Error(`daemon spawner exited ${spawnerExit}; see ${daemonLogPath}`);
    }
    const daemonPid = Number(spawnerOut.trim());
    if (!daemonPid) {
      throw new Error(`daemon spawner reported no pid; see ${daemonLogPath}`);
    }
    // Re-point the lock at the daemon: it now outlives this client, and a
    // lock keyed to a dead client's pid would be stolen by the next spawner
    // while the daemon is still starting up, yielding two daemons on one
    // socket path.
    const daemonHolder = await holderLine(daemonPid).catch(() => null);
    if (!daemonHolder) {
      throw new Error(`daemon exited during startup; see ${daemonLogPath}`);
    }
    await fs.writeFile(context.lockPath, daemonHolder);
    // Generous timeout: first spawn may sit through an npx download of codex.
    const timeoutMs = Number(process.env.CEPTION_SPAWN_TIMEOUT_SECS ?? 120) * 1000;
    await waitForSocket(context.socketPath, timeoutMs, () =>
      pidAlive(daemonPid)
        ? null
        : new Error(`daemon exited during startup; see ${daemonLogPath}`)
    );
    return { spawned: true };
  } finally {
    await release();
  }
}

async function ensureLiveDaemon(context, options = {}) {
  const live = await daemonStatus(context);
  if (live) {
    return { spawned: false, daemon: live };
  }
  await fs.rm(context.socketPath, { force: true }).catch(() => {});
  const spawnResult = await spawnDaemon(context, options);
  const daemon = await daemonStatus(context);
  if (!daemon) {
    throw new Error("daemon did not become ready");
  }
  return { ...spawnResult, daemon };
}

function printReportAndSetExit(result, label = null) {
  if (result.report) {
    process.stdout.write(`${result.report.replace(/\s+$/u, "")}\n`);
  }
  // Turn reports say what happened in the turn; the goal says whether codex is
  // going to keep going. Both are needed to know if the run is over.
  if (result.turnId && result.goal) {
    process.stdout.write(`\n${formatGoal(result.goal, label)}\n`);
  }
  process.exitCode = statusExitCode(result.status);
}

// Find this label under another session and, if its owner is gone, move it
// into ours. Labels owned by a live session are never touched: that would be
// two sessions sharing one rollout.
async function adoptLabel(context) {
  const rows = (await listProjectLabels(context)).filter(
    (row) => row.label === context.label && row.sessionKey !== context.sessionKey && row.threadId
  );
  rows.sort((a, b) => (b.lastUsed ?? "").localeCompare(a.lastUsed ?? ""));
  for (const row of rows) {
    if (await sessionAlive(row.sessionKey)) {
      throw usageError(
        `label ${context.label} belongs to live session ${row.sessionKey}; use a different label`
      );
    }
    if (row.sessionKey === "default" && (await daemonStatus(row))) {
      throw usageError(
        `label ${context.label} has a live daemon outside any Claude session; use a different label or kill it there`
      );
    }
    const entry = await takeLabel(context, row.sessionKey);
    if (entry) {
      return entry;
    }
  }
  return null;
}

export async function cliSpawn({ label, cwd, prompt, model, effort, report }) {
  const context = await contextFor(cwd, label);
  await gcProjectState(context).catch(() => {});
  if (await daemonStatus(context)) {
    throw usageError(`daemon already live for ${label}`);
  }
  await fs.rm(context.socketPath, { force: true }).catch(() => {});
  const spawnResult = await spawnDaemon(context, { model, effort });
  if (!spawnResult.spawned) {
    throw usageError(`daemon already live for ${label}`);
  }
  process.stdout.write(`log: ${context.logPath}\n`);
  const result = await commandDaemon(context.socketPath, { cmd: "turn", prompt, report });
  printReportAndSetExit(result, label);
}

// Reach the label's daemon, respawning it against the stored thread if it died
// (or adopting the label from a dead session). Returns the log path to print.
async function reviveDaemon(context, label) {
  const daemon = await daemonStatus(context);
  if (daemon) {
    return daemon.logPath ?? context.logPath;
  }
  await fs.rm(context.socketPath, { force: true }).catch(() => {});
  let labelState = await readLabelState(context);
  if (!labelState?.threadId) {
    labelState = await adoptLabel(context);
  }
  if (!labelState?.threadId) {
    throw usageError(
      `no live daemon or stored thread for ${label} in project ${context.cwd} (session ${context.sessionKey}); labels resolve from the invocation cwd — pass --cwd, or run \`ception list --all\` to see every project`
    );
  }
  await ensureLiveDaemon(context, {
    resumeThreadId: labelState.threadId,
    model: labelState.model ?? null,
    effort: labelState.effort ?? null,
    logPath: labelState.logPath ?? null
  });
  return labelState.logPath ?? context.logPath;
}

export async function cliSend({ label, cwd, prompt, report }) {
  const context = await contextFor(cwd, label);
  await gcProjectState(context).catch(() => {});
  const logPath = await reviveDaemon(context, label);
  process.stdout.write(`log: ${logPath}\n`);
  // The daemon decides atomically: steer if a turn is active, else new turn.
  const result = await commandDaemon(context.socketPath, { cmd: "send", prompt, report });
  printReportAndSetExit(result, label);
}

// `goal` sets codex's own long-run driver. Setting or resuming one blocks on
// the turn codex starts for it, like spawn/send; pause, clear and show return
// at once. A goal on a label with no daemon yet starts one: codex works from
// the objective alone, with no opening prompt.
export async function cliGoal({ label, cwd, action, objective, report }) {
  const context = await contextFor(cwd, label);
  await gcProjectState(context).catch(() => {});
  const live = await daemonStatus(context);
  const fresh = !live && action === "set" && !(await readLabelState(context))?.threadId;
  let logPath = live?.logPath ?? context.logPath;
  if (fresh) {
    // A label with no thread is legitimate here: an objective is enough to
    // start a run, so give it a daemon rather than demanding a `spawn` first.
    await fs.rm(context.socketPath, { force: true }).catch(() => {});
    await ensureLiveDaemon(context, {});
  } else if (!live) {
    logPath = await reviveDaemon(context, label);
  }
  if (action === "set" || action === "resume") {
    process.stdout.write(`log: ${logPath}\n`);
  }
  const result = await commandDaemon(context.socketPath, { cmd: "goal", action, objective, report });
  printReportAndSetExit(result, label);
}

export async function cliInterrupt({ label, cwd }) {
  const context = await contextFor(cwd, label);
  const live = await daemonStatus(context);
  if (!live) {
    process.stdout.write("no active turn\n");
    return;
  }
  const result = await commandDaemon(context.socketPath, { cmd: "interrupt" });
  printReportAndSetExit(result, label);
}

export async function cliKill({ label, all, cwd }) {
  if (all) {
    const context = await contextFor(cwd, "_probe");
    const rows = (await listProjectLabels(context)).filter(
      (row) => row.sessionKey === context.sessionKey
    );
    let killed = 0;
    for (const row of rows) {
      const live = await daemonStatus(row);
      if (!live) {
        continue;
      }
      await commandDaemon(row.socketPath, { cmd: "shutdown" }).catch(() => {});
      killed += 1;
    }
    process.stdout.write(`killed ${killed} daemon(s)\n`);
    return;
  }

  const context = await contextFor(cwd, label);
  const live = await daemonStatus(context);
  if (!live) {
    await fs.rm(context.socketPath, { force: true }).catch(() => {});
    process.stdout.write("no live daemon\n");
    return;
  }
  const result = await commandDaemon(context.socketPath, { cmd: "shutdown" });
  printReportAndSetExit(result);
}

export async function cliList({ all, json, cwd }) {
  const context = await contextFor(cwd, "_probe");
  await gcProjectState(context).catch(() => {});
  const rows = all ? await listAllLabels() : await listProjectLabels(context);

  const enriched = [];
  for (const row of rows) {
    const live = await daemonStatus(row);
    const mine = row.sessionKey === context.sessionKey;
    let status;
    if (live) {
      status = live.state;
    } else if (mine || (await sessionAlive(row.sessionKey))) {
      status = "dead";
    } else {
      status = "adoptable";
    }
    enriched.push({
      label: row.label,
      cwd: row.cwd,
      hash: row.hash,
      session: mine ? "mine" : row.sessionKey,
      status,
      goal: live?.goal?.status ?? null,
      threadId: live?.threadId ?? row.threadId ?? null,
      lastUsed: row.lastUsed ?? null,
      logPath: row.logPath ?? null
    });
  }

  if (json) {
    process.stdout.write(`${JSON.stringify(enriched, null, 2)}\n`);
    return;
  }
  for (const row of enriched) {
    process.stdout.write(
      `${row.label}\t${row.session}\t${row.status}\tgoal=${row.goal ?? "-"}\t${row.threadId ?? "-"}\t${row.lastUsed ?? "-"}\t${row.logPath ?? "-"}\n`
    );
  }
}

export async function cliWatch({ label, cwd, report, follow }) {
  const context = await contextFor(cwd, label);

  if (follow) {
    let state = await readLabelState(context);
    if (!state?.logPath) {
      // Cross-session peek is fine for a read-only tail.
      const rows = (await listProjectLabels(context)).filter((row) => row.label === label);
      rows.sort((a, b) => (b.lastUsed ?? "").localeCompare(a.lastUsed ?? ""));
      state = rows[0] ?? null;
    }
    const logPath = state?.logPath ?? context.logPath;
    await fs.mkdir(path.dirname(logPath), { recursive: true });
    const child = spawn("tail", ["-F", logPath], { stdio: "inherit" });
    await new Promise((resolve) => child.on("exit", resolve));
    return;
  }

  // Attach to the daemon and block until the current turn completes; the
  // report arrives exactly as it would have on the spawn/send client. Idle
  // daemons answer immediately, so a watcher never hangs on a finished turn.
  const daemon = await daemonStatus(context);
  if (!daemon) {
    throw usageError(
      `no live daemon for ${label}; \`ception send\` respawns and resumes a stored thread`
    );
  }
  process.stdout.write(`log: ${daemon.logPath}\n`);
  const result = await commandDaemon(context.socketPath, { cmd: "watch", report });
  printReportAndSetExit(result, label);
}

// Intermediate leg of spawnDaemon: start the daemon detached, report its pid
// on stdout, exit. Our prompt exit orphans the daemon onto init.
export async function cliSpawnOrphan(argv) {
  const logIndex = argv.indexOf("--log");
  const daemonLogPath = argv[logIndex + 1];
  if (logIndex === -1 || !daemonLogPath) {
    throw new Error("spawn-orphan missing --log");
  }
  const logFd = fsSync.openSync(daemonLogPath, "a");
  const child = spawn(process.execPath, [BIN_PATH, "daemon", ...argv], {
    detached: true,
    stdio: ["ignore", logFd, logFd]
  });
  child.unref();
  fsSync.closeSync(logFd);
  process.stdout.write(`${child.pid}\n`);
}

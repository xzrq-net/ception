...
// Locks are published by link(2) so the pid is in the file the moment it
// exists; a lock without a readable live pid is debris, never a holder.
async function acquireLock(context, timeoutMs = 15000) {
  const tempPath = `${context.lockPath}.${process.pid}.tmp`;
  await fs.writeFile(tempPath, `${process.pid}\n`, { mode: 0o600 });
  const deadline = Date.now() + timeoutMs;
  try {
    while (Date.now() < deadline) {
      try {
        await fs.link(tempPath, context.lockPath);
        return async () => {
...
async function spawnDaemon(context, { resumeThreadId = null, model = null, effort = null, logPath = null } = {}) {
  ...
  try {
    ...
    // socket path.
    await fs.writeFile(context.lockPath, `${daemonPid}\n`);

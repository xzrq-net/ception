...
async function commandDaemon(socketPath, command, { onWarning } = {}) {
  const socket = await socketConnect(socketPath);
...
async function spawnDaemon(context, { resumeThreadId = null, model = null, effort = null, sandbox = "workspace-write" } = {}) {
...
export async function cliSpawn({ label, cwd, prompt, model, effort, sandbox, report }) {
  ...
  await fs.rm(context.socketPath, { force: true }).catch(() => {});
  const spawnResult = await spawnDaemon(context, { model, effort, sandbox });
  ...
  const result = await commandDaemon(
    context.socketPath,
    { cmd: "turn", prompt, report },
    { onWarning: (message) => process.stdout.write(`warning: ${message}\n`) }
  );
...
export async function cliSend({ label, cwd, prompt, report }) {
  ...
  if (!daemon) {
    ...
    await ensureLiveDaemon(context, {
      ...
      sandbox: labelState.sandbox ?? "workspace-write"
    });
